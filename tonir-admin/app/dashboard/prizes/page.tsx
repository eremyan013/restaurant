...
export default async function PrizesPage() {
  ...
  const [prizesRes, claimsRes, venuesRes, settingsRes] = await Promise.all([
    supabase.from('prizes').select('id, name, description, type, unlock_type, points_cost, min_tier_level, venue_id, image_url, stock, sort_order, is_active, created_at, updated_at').order('sort_order').order('created_at', { ascending: false }),
